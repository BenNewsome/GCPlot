...
def main(species,group,debug):
   ...
   species_data, units, start_time, end_time = get_species_data_from_bpch(wd1,species,group, month)
   print 'Species data dimenstion = ' + str(species_data.dimensions)

   relevant_data1 = strip_relevant_data(species_data,debug)
   print 'relevant data dimensions = ' + str(species_data.dimensions   )

   #Get the second one
   species_data, units, start_time, end_time = get_species_data_from_bpch(wd2,species,group, month)
   relevant_data2 = strip_relevant_data(species_data,debug)

   #Get the difference
   relevant_data = 100 * ((relevant_data2 - relevant_data1) / relevant_data1)



   plot_the_data(relevant_data, units, start_time, end_time, Output_Name, debug)
   return;
...
def plot_the_data(data, units, start_time, end_time, Output_Name, debug):
...
#   print data.dimensions
   ...
   Y_axis_size             = 25
   Plot_title              =  "$-\sigma$ difference in average surface " + species + ' between '+ str(start_time) + ' and ' + str(end_time)
